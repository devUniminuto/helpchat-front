...
export default {
  ...
  created: function created() {
    ...
    var needsSchemaGrid = this.grid === true || this.actionButtons.includes('grid');

    if (needsSchemaGrid && this.schemaGrid === undefined) {
      throw new Error('[BlitzTable] You need to defined a prop called "schemaGrid" if you want to use a grid layout as well!');
    }
  },
  mounted: function mounted() {
    var footerEl = this.$el.querySelector('.q-table__bottom');
    if (footerEl) footerEl.classList.add('blitz-table__footer');
    var topEl = this.$el.querySelector('.q-table__top');
    if (topEl) topEl.classList.add('blitz-table__top');
    var titleEl = this.$el.querySelector('.q-table__title');
  ...
  data: function data() {
    ...
    var gridModeEnabled = grid; // const innerFilter = filter
  ...
  computed: {
    selectionMode: function selectionMode() {
      var qTableProps = this.qTableProps;
      return qTableProps.selection === 'single' || qTableProps.selection === 'multiple';
    },
    allRowsAreSelected: function allRowsAreSelected() {
      var rows = this.rows,
          cSelected = this.cSelected;
    ...
    pagination: {
      ...
      set: function set(newPagination) {
    ...
    usesTopSlot: function usesTopSlot() {
      var title = this.title,
          cActionButtons = this.cActionButtons,
          slot = this.$scopedSlots;
      return title || cActionButtons.length || slot['above-table'] || slot['above-nav-row'] || slot['top-left'] || slot['top-right'];
    },
    qTableProps: function qTableProps() {
      var propsToNotPass = ['cardStyle', 'cardClass'];
      var propsToPass = merge(this.$attrs, {
        // Quasar props with modified behavior:
        data: this.rows,
        columns: this.cColumns,
        rowKey: 'id',
        grid: this.gridModeEnabled // Quasar props with modified defaults:
        // filter: this.$attrs.filter || this.innerFilter,
        // Quasar props just to pass:

      });
      return Object.entries(propsToPass).reduce(function (carry, _ref) {
    ...
    gridBlitzFormProps: function gridBlitzFormProps() {
      var gridBlitzFormOptions = this.gridBlitzFormOptions,
    ...
    cSelected: {
      ...
      set: function set(val) {
        // set indeterminate
        var isIndeterminate = val.length && val.length < this.rows.length;
        var headerCheckbox = this.$el.querySelector('.js-blitz-header-selection input');
    ...
    cColumns: function cColumns() {
      return schemaToQTableColumns(this.schemaColumns);
    },
    cActionButtons: function cActionButtons() {
      var _this = this;

      var actionButtons = this.actionButtons,
          schemaGrid = this.schemaGrid;
      var blitzTableContext = this;
      var defaultsGridButton = {
        component: BlitzGridListToggle,
        value: this.gridModeEnabled,
        events: {
          input: function input(newVal) {
            return _this.gridModeEnabled = newVal;
          }
        }
      };
      return actionButtons.map(function (btn) {
        ...
        if (isPlainObject(btn)) {
          ...
          var click = btn.events.click;
          if (isFunction(click)) btn.events.click = function (val) {
            return click(val, blitzTableContext);
          };
          return btn;
        }
      }).filter(function (btn) {
        return isPlainObject(btn);
  ...
  methods: {
    evaluate: function evaluate(propValue, rowProps) {
      if (!isFunction(propValue)) return propValue || '';
      return propValue(rowProps.row, rowProps, this) || '';
    },
    setSelectionAllRows: function setSelectionAllRows(setTo) {
      if (setTo === true) {
        var _this$$refs$qtable;

        var filteredRowsFromQTable = (_this$$refs$qtable = this.$refs.qtable) === null || _this$$refs$qtable === void 0 ? void 0 : _this$$refs$qtable.filteredSortedRows;
        this.cSelected = isArray(filteredRowsFromQTable) ? filteredRowsFromQTable : this.rows;
      } else {
        this.cSelected = [];
      }
    },
    onCellDblclick: function onCellDblclick(event, rowData, colId) {
      this.event('row-dblclick', event, rowData);
      this.event('cell-dblclick', event, rowData, colId);
    },
    onCellClick: function onCellClick(event, rowData, colId) {
      this.onRowClick(event, rowData);
      this.event('cell-click', event, rowData, colId);
    },
    onRowClick: function onRowClick(event, rowData, origin, gridItemProps) {
    ...
    onInputCell: function onInputCell(rowId, colId, value, origin) {
      this.event('input-cell', {
        rowId: rowId,
        colId: colId,
        value: value,
        origin: origin
    ...
    event: function event(eventName) {
      ...
      if (eventName === 'update:pagination') {
         ...
         */
        this.$emit.apply(this, ['update:pagination'].concat(args));
      }
      ...
      if (eventName === 'update:selected') {
         ...
         */
        this.$emit.apply(this, ['update:selected'].concat(args));
      }
      ...
      if (eventName === 'row-click') {
         ...
         */
        this.$emit.apply(this, ['row-click'].concat(args));
      }
      ...
      if (eventName === 'row-dblclick') {
         ...
         * @property {Record<string, any>} payload the rowData
         */
        this.$emit.apply(this, ['row-dblclick'].concat(args));
      }
      ...
      if (eventName === 'cell-click') {
         ...
         */
        this.$emit.apply(this, ['cell-click'].concat(args));
      }
      ...
      if (eventName === 'cell-dblclick') {
         ...
         * @property {string} colId the column ID, this is what you have set as `id` in the schema
         */
        this.$emit.apply(this, ['cell-dblclick'].concat(args));
      }
      ...
      if (eventName === 'input-cell') {
         ...
         */
        this.$emit.apply(this, ['input-cell'].concat(args));
      }
    }
  }
};
...
.blitz-table ._table-top-right,
.blitz-table ._table-selection-cell {
  display: flex;
...
.blitz-table .q-table__top {
  display: flex;
  flex-wrap: nowrap;
  flex-direction: column;
  align-items: stretch;
}
.blitz-table.q-table--grid .q-table__middle {
  min-height: 0;
  margin-bottom: 0;
}
...
.blitz-table__grid-item .blitz-field__sub-label {
  display: none;
}
